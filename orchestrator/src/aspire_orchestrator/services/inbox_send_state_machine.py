"""Inbox Send State Machine — 5-state email-send lifecycle.

States: DRAFT -> APPROVED -> SENDING -> SENT -> FAILED
...
Governance guarantees:
  ...
  - DRAFT->APPROVED requires YELLOW approval evidence (Law #4)
  - SENDING has a 60s timeout (enforced externally, warning in receipt)
...
class TransitionReceipt:
    ...
    Append-only. No UPDATE/DELETE. Corrections are new receipts.
    """
...
# Timeout for SENDING state (seconds)
SENDING_TIMEOUT_SECONDS = 60


class InboxSendStateMachine:
    """5-state email-send lifecycle engine.
    ...
    def transition(
        ...
        *,
        ...
        Args:
            entity_id: The entity being transitioned (must match self).
        ...
        # CHECK: YELLOW tier requires approval_evidence (Law #4)
        ...
        logger.info(
            "InboxSend %s transitioned %s -> %s (actor=%s, corr=%s, tier=%s)",
