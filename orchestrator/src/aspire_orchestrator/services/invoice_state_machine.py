"""Invoice State Machine — 7-state invoice lifecycle.

States: DRAFT -> APPROVED -> SENT -> VIEWED -> PAID -> VOID -> REFUNDED
...
Governance guarantees:
  ...
  - DRAFT->APPROVED requires YELLOW approval with binding_fields (Law #4)
  - PAID->REFUNDED requires RED tier with presence_token (Law #4)
  - VOID reachable from any non-terminal state (except REFUNDED)
  - Terminal states: VOID, REFUNDED
...
# Required binding fields for DRAFT->APPROVED
APPROVAL_BINDING_FIELDS: frozenset[str] = frozenset({"recipient", "amount", "line_items"})
...
# VOID is reachable from any non-terminal state (except REFUNDED)
...
class InvoiceStateMachine:
    """7-state invoice lifecycle engine.
    ...
    def transition(
        ...
        *,
        ...
        Args:
            entity_id: The invoice being transitioned (must match self).
            ...
            approval_evidence: Required for YELLOW/RED tiers (Law #4).
                For DRAFT->APPROVED, must contain binding_fields: recipient, amount, line_items.
        ...
        # CHECK: DRAFT->APPROVED requires binding_fields in approval_evidence
        if from_state == "draft" and to_state == "approved" and approval_evidence:
            missing_fields = APPROVAL_BINDING_FIELDS - set(approval_evidence.keys())
            if missing_fields:
                denial = TransitionReceipt(
                    correlation_id=correlation_id,
                    suite_id=suite_id,
                ...
                self._history.append(denial)
                raise InvalidTransitionError(
                    f"DRAFT->APPROVED requires binding fields {APPROVAL_BINDING_FIELDS}. "
        ...
        logger.info(
            "Invoice %s transitioned %s -> %s (actor=%s, corr=%s, tier=%s)",
