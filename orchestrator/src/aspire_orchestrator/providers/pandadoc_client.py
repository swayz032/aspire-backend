...
async def _llm_fill_missing_tokens(
    ...
    try:
        ...
        if json_start >= 0 and json_end > json_start:
            raw_values = json.loads(content[json_start:json_end])
...
async def _build_pricing_tables(
    ...
    # Determine pricing table name (per-template override or PandaDoc default)
    table_name = "Pricing Table 1"
    if template_spec and template_spec.get("pricing_table_name"):
...
async def _build_content_placeholders(
    ...
    try:
        ...
        arr_end = content.rfind("]") + 1
        if arr_start >= 0 and arr_end > arr_start:
            result = json.loads(content[arr_start:arr_end])
...
async def execute_pandadoc_contract_generate(
    *,
    ...
    if auto_tokens and all_missing:
        ...
        # Gate: block creation if fill rate < 40% OR critical tokens missing >= 3
        if fill_rate < 40 or len(critical_missing) >= 3:
