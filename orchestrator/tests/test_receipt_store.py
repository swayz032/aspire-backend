...
class TestInMemoryStore:
    ...
    def test_store_and_query_basic(self):
        receipts = [
            {"id": "r1", "suite_id": "s1", "action_type": "calendar.read", "created_at": "2026-01-01T00:00:00Z"},
            {"id": "r2", "suite_id": "s1", "action_type": "invoice.create", "created_at": "2026-01-02T00:00:00Z"},
            {"id": "r3", "suite_id": "s2", "action_type": "calendar.read", "created_at": "2026-01-03T00:00:00Z"},
        ]
        store_receipts(receipts)

        # Query by suite_id (Law #6: tenant isolation)
        result = query_receipts(suite_id="s1")
        assert len(result) == 2
        assert all(r["suite_id"] == "s1" for r in result)
    ...
    def test_query_with_filters(self):
        receipts = [
            {"id": "r1", "suite_id": "s1", "action_type": "calendar.read", "risk_tier": "green",
             "correlation_id": "c1", "created_at": "2026-01-01T00:00:00Z"},
            {"id": "r2", "suite_id": "s1", "action_type": "invoice.create", "risk_tier": "yellow",
             "correlation_id": "c2", "created_at": "2026-01-02T00:00:00Z"},
        ]
        store_receipts(receipts)

        # Filter by action_type
        result = query_receipts(suite_id="s1", action_type="invoice.create")
        assert len(result) == 1
        assert result[0]["id"] == "r2"

        # Filter by risk_tier
        result = query_receipts(suite_id="s1", risk_tier="green")
        assert len(result) == 1
        assert result[0]["id"] == "r1"

        # Filter by correlation_id
        result = query_receipts(suite_id="s1", correlation_id="c1")
        assert len(result) == 1
        assert result[0]["id"] == "r1"

    def test_query_pagination(self):
        receipts = [
            {"id": f"r{i}", "suite_id": "s1", "created_at": f"2026-01-{i+1:02d}T00:00:00Z"}
            for i in range(10)
        ]
        store_receipts(receipts)

        result = query_receipts(suite_id="s1", limit=3, offset=0)
        assert len(result) == 3

        result = query_receipts(suite_id="s1", limit=3, offset=7)
        assert len(result) == 3

    def test_query_sorted_newest_first(self):
        receipts = [
            {"id": "r-old", "suite_id": "s1", "created_at": "2026-01-01T00:00:00Z"},
            {"id": "r-new", "suite_id": "s1", "created_at": "2026-01-05T00:00:00Z"},
            {"id": "r-mid", "suite_id": "s1", "created_at": "2026-01-03T00:00:00Z"},
        ]
        store_receipts(receipts)
    ...
    def test_get_chain_receipts(self):
        receipts = [
            {"id": "r1", "suite_id": "s1", "chain_id": "s1", "sequence": 2},
            {"id": "r2", "suite_id": "s1", "chain_id": "s1", "sequence": 1},
            {"id": "r3", "suite_id": "s1", "chain_id": "other", "sequence": 1},
        ]
        store_receipts(receipts)

        chain = get_chain_receipts(suite_id="s1", chain_id="s1")
        assert len(chain) == 2
        assert chain[0]["sequence"] == 1  # sorted ascending
        assert chain[1]["sequence"] == 2

    def test_get_receipt_count(self):
        store_receipts([{"id": "r1", "suite_id": "s1"}, {"id": "r2", "suite_id": "s2"}])
        assert get_receipt_count() == 2
        assert get_receipt_count("s1") == 1
        assert get_receipt_count("s999") == 0

    def test_clear_store(self):
        store_receipts([{"id": "r1", "suite_id": "s1"}])
...
class TestDualWrite:
    ...
    def test_persist_gracefully_handles_no_client(self, mock_get_client):
        ...
        from aspire_orchestrator.services.receipt_store import _persist_to_supabase
        # Should not raise
        _persist_to_supabase([{"id": "r1", "suite_id": "s1"}])
...
class TestImmutability:
    ...
    def test_store_is_append_only(self):
        store_receipts([{"id": "r1", "suite_id": "s1"}])
        store_receipts([{"id": "r2", "suite_id": "s1"}])
        assert get_receipt_count() == 2
