...
def test_adam_research_validates() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "adam_research", "owner_key": None, "manifest_id": "adam-research"})(),
    )
    problems = module.validate_agent(ROOT, target)
    assert problems == []


def test_adam_research_certifies() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "adam_research", "owner_key": None, "manifest_id": "adam-research"})(),
    )
