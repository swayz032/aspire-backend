...
def test_teressa_books_validates() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "teressa_books", "owner_key": None, "manifest_id": "teressa-books"})(),
    )
    problems = module.validate_agent(ROOT, target)
    assert problems == []


def test_teressa_books_certifies() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "teressa_books", "owner_key": None, "manifest_id": "teressa-books"})(),
    )
