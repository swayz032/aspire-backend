...
def test_finn_finance_manager_validates() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "finn_finance_manager", "owner_key": None, "manifest_id": "finn-finance-manager"})(),
    )
    problems = module.validate_agent(ROOT, target)
    assert problems == []


def test_finn_finance_manager_certifies() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "finn_finance_manager", "owner_key": None, "manifest_id": "finn-finance-manager"})(),
    )
