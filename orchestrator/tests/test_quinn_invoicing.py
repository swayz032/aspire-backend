...
def test_quinn_invoicing_validates() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "quinn_invoicing", "owner_key": None, "manifest_id": "quinn-invoicing"})(),
    )
    problems = module.validate_agent(ROOT, target)
    assert problems == []


def test_quinn_invoicing_certifies() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "quinn_invoicing", "owner_key": None, "manifest_id": "quinn-invoicing"})(),
    )
