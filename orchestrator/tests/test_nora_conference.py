...
def test_nora_conference_validates() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "nora_conference", "owner_key": None, "manifest_id": "nora-conference"})(),
    )
    problems = module.validate_agent(ROOT, target)
    assert problems == []


def test_nora_conference_certifies() -> None:
    target = module.build_validation_target(
        ROOT,
        type("Args", (), {"registry_id": "nora_conference", "owner_key": None, "manifest_id": "nora-conference"})(),
    )
