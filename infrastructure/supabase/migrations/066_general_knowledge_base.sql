-- Migration 066: General Knowledge Base for Ava RAG
-- pgvector-backed hybrid search (vector + full-text + metadata filtering)
-- Supports domains: aspire_platform, business_operations, industry_knowledge, best_practices
-- RLS: global knowledge (suite_id IS NULL) visible to all, tenant-specific scoped by suite_id

-- ============================================================================
-- Table: general_knowledge_chunks — Primary retrieval table
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.general_knowledge_chunks (
    ...
    chunk_type TEXT CHECK (chunk_type IN (
        'concept', 'procedure', 'definition', 'example',
        'best_practice', 'faq', 'checklist', 'tip',
        'industry_insight', 'platform_feature'
    )),
    chunk_index INT,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_general_chunk_content_domain_tenant
    ON public.general_knowledge_chunks(content_hash, domain, COALESCE(suite_id, '00000000-0000-0000-0000-000000000000'::uuid));
...
-- ============================================================================
-- Table: general_knowledge_sources — Tracks ingestion sources
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.general_knowledge_sources (
    ...
    source_type TEXT NOT NULL CHECK (source_type IN (
        'file', 'url', 'api', 'manual', 'platform_docs'
    )),
...
CREATE INDEX IF NOT EXISTS idx_general_sources_domain
    ON public.general_knowledge_sources(domain);
CREATE INDEX IF NOT EXISTS idx_general_sources_sync_status
    ON public.general_knowledge_sources(sync_status);
CREATE INDEX IF NOT EXISTS idx_general_sources_suite_id
    ON public.general_knowledge_sources(suite_id);
...
-- ============================================================================
-- Function: search_general_knowledge() — Hybrid vector + full-text search
-- ============================================================================
...
COMMENT ON FUNCTION public.search_general_knowledge IS
    'Hybrid vector + full-text search for Ava general RAG. Supports domain and tenant filtering.';

COMMENT ON TABLE public.general_knowledge_chunks IS
    'Ava general RAG knowledge base chunks. pgvector embeddings + full-text search. RLS: global + tenant-scoped.';
COMMENT ON COLUMN public.general_knowledge_chunks.suite_id IS
