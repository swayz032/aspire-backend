-- Migration 067: Communication Knowledge Base for Eli RAG
-- pgvector-backed hybrid search (vector + full-text + metadata filtering)
-- Supports domains: email_best_practices, client_communication, business_writing, tone_guidance
-- RLS: global knowledge (suite_id IS NULL) visible to all, tenant-specific scoped by suite_id

-- ============================================================================
-- Table: communication_knowledge_chunks — Primary retrieval table
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.communication_knowledge_chunks (
    ...
    chunk_type TEXT CHECK (chunk_type IN (
        'template', 'example', 'guideline', 'definition',
        'best_practice', 'faq', 'checklist', 'tone_rule',
        'subject_line', 'follow_up_pattern'
    )),
    chunk_index INT,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_comm_chunk_content_domain_tenant
    ON public.communication_knowledge_chunks(content_hash, domain, COALESCE(suite_id, '00000000-0000-0000-0000-000000000000'::uuid));
...
-- ============================================================================
-- Table: communication_knowledge_sources — Tracks ingestion sources
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.communication_knowledge_sources (
    ...
    source_type TEXT NOT NULL CHECK (source_type IN (
        'file', 'url', 'api', 'manual', 'style_guide'
    )),
...
CREATE INDEX IF NOT EXISTS idx_comm_sources_domain
    ON public.communication_knowledge_sources(domain);
CREATE INDEX IF NOT EXISTS idx_comm_sources_sync_status
    ON public.communication_knowledge_sources(sync_status);
CREATE INDEX IF NOT EXISTS idx_comm_sources_suite_id
    ON public.communication_knowledge_sources(suite_id);
...
-- ============================================================================
-- Function: search_communication_knowledge() — Hybrid vector + full-text search
-- ============================================================================
...
COMMENT ON FUNCTION public.search_communication_knowledge IS
    'Hybrid vector + full-text search for Eli communication RAG. Supports domain and tenant filtering.';

COMMENT ON TABLE public.communication_knowledge_chunks IS
    'Eli communication RAG knowledge base chunks. pgvector embeddings + full-text search. RLS: global + tenant-scoped.';
COMMENT ON COLUMN public.communication_knowledge_chunks.suite_id IS
